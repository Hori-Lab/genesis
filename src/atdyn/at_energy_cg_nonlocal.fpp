...
contains
  ...
  subroutine compute_energy_CG_ele(enefunc, pairlist, coord, force, virial, &
    ...
    ele_tmp_a_C   = 1.0_wp - 2.551e-1_wp * ele_tmp_sol_C  &
  ...
  subroutine compute_energy_CG_IDR_HPS(enefunc, pairlist, coord, force, virial, ehps)
    ...
    integer                   :: i, j, k, l, m
    real(wp)                  :: lambda, epsilon, epsilon_tmp
    real(wp)                  :: epsilon_aromatic, epsilon_cationpi
    real(wp)                  :: sigma, sigma_sqr, sigma_6th
    real(wp)                  :: dij(3), rij_sqr
    real(wp)                  :: cutoff, cutoff_sqr
    real(wp)                  :: inv_rij_sqr, inv_rij_6th
    ...
    epsilon_tmp      =  enefunc%cg_IDR_HPS_epsilon
    ...
    do i = 1, natom-1
      ...
      do k = ini_hps, fin_hps
        ...
        ! aromatic interactions ff < yy < ww ? from MPIPI paper
          ...
          !cg_IDR_HPS_para_epsilon(i,j) = 0.9_wp
          ...
          epsilon = epsilon_aromatic
          !cg_IDR_HPS_para_epsilon(i,j) = 0.9_wp
        else if ((protein_residue(i) == 'PHE' .and. protein_residue(j) == 'TYR') &
        .or.     (protein_residue(i) == 'TYR' .and. protein_residue(j) == 'PHE')) then
          lambda  = 0.451205_wp
          epsilon = epsilon_aromatic
          !cg_IDR_HPS_para_epsilon(i,j) = 0.9_wp
        else if ((protein_residue(i) == 'PHE' .and. protein_residue(j) == 'TRP') &
        .or.     (protein_residue(i) == 'TRP' .and. protein_residue(j) == 'PHE')) then
          lambda  = 0.537465_wp
          epsilon = epsilon_aromatic
          !cg_IDR_HPS_para_epsilon(i,j) = 0.9_wp
        else if ((protein_residue(i) == 'TYR' .and. protein_residue(j) == 'TRP') &
        .or.     (protein_residue(i) == 'TRP' .and. protein_residue(j) == 'TYR')) then
          lambda  = 0.554138_wp
          epsilon = epsilon_aromatic
